...
class ScalAT(problemName: String = "", workingpath: String = "working/") {
  ...
  //It is required that we have previously called teh solve() method and that it has returned true
  ...
  def addAMOLog(x: List[Int]): Unit = {
    //We declare the new K variables (log_2(x.size))
    val k: Int = math.ceil(math.log(x.length) / math.log(2)).toInt
    val auxiliar: Array[Int] = newVarArray(k)

    //Once we have the variables, we encode the at-most-one
    for(i <- x.indices){
      val binaryIndexes =  i.toBinaryString.reverse.padTo(k, '0').reverse
      for(j <- binaryIndexes.indices){
        if(binaryIndexes(j) == '0'){
          addClause(-x(i) :: auxiliar(j) :: List())
        }
        else{
          addClause(-x(i) :: -auxiliar(j) :: List())
        }
  ...
  //Ads the encoding of "sort x decreasingly into y". Both lists must have equal length, and empty lists are not allowed
